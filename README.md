...
Кросс-браузерное расширение для Chromium и Firefox/LibreWolf. Расширение локально сканирует открытую страницу МЭШ, собирает найденные карточки заданий и показывает их в едином интерфейсе.
...
Открой `about:debugging#/runtime/this-firefox`, нажми **«Загрузить временное дополнение»** и выбери `dist/firefox/manifest.json`. После перезапуска LibreWolf временное дополнение может потребоваться загрузить повторно.
