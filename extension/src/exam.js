...
function buildSubmitPayload(task) {
  ...
  if (type === "answer/string/multiple") { const answers = Array.isArray(value) ? value.map((item) => String(item ?? "").trim()).filter(Boolean) : []; return answers.length ? { "@answer_type": type, case_sensitive: false, quantity: answers.length, with_order: false, answers: answers.map((text, num) => ({ id: crypto.randomUUID(), strings: [text], num, group: 0 })) } : null; }
  ...
  if (type === "answer/groups") {
    ...
    return { "@answer_type": type, groups: Object.entries(grouped).map(([group_id, options_ids]) => ({ group_id, options_ids })) };
  }
...
function normalizeImportedAnswer(task, value) {
  ...
  if (type === "answer/inline/choice/single") {
    ...
    return Object.fromEntries(entries.map(([key, item]) => { const selected = item && typeof item === "object" ? item.id ?? item.value ?? item.text : item; const position = positions.find((candidate, index) => String(candidate?.position_id ?? index) === String(key)); const id = findOptionId({ answer: { options: position?.options || [] } }, selected); if (id == null) throw new Error(`вариант ${selected} не найден в позиции ${key} задания ${task.id}`); return [String(position?.position_id ?? key), { text_id: String(item?.text_id ?? position?.text_id ?? ""), id }]; }));
  }
  return value;
}
