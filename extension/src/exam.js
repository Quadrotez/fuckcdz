...
document.querySelector("#prompt").addEventListener("click", () => { if (!state.snapshot?.response) return alert("Сначала открой тест и начни попытку."); const payload = exportPayload(); payload.answers = {}; const instructions = `Ты решаешь учебный тест. Верни только корректный JSON без Markdown и пояснений в формате:\n{\n  "answers": {\n    "ID задания": "ID варианта или текстовый ответ"\n  }\n}\n\nПравила: answer/single — строковый id варианта; answer/multiple — массив id; answer/free и answer/string — строка; answer/number — число; answer/order — массив id в правильном порядке; answer/match и answer/groups — объект соответствий; answer/gap/text/input — объект с номерами полей и вставляемыми значениями. Не добавляй right_answer и не меняй ID заданий. Если данных недостаточно, оставь значение пустым, но сохрани JSON-структуру.`; document.querySelector("#prompt-text").value = `${instructions}\n\nДанные теста:\n${JSON.stringify(payload, null, 2)}`; document.querySelector("#prompt-dialog").showModal(); });
